...
public class BanCommand implements EchobotCommand {
    ...
    public void onMessageCreate(MessageCreateEvent e) {
        ...
        if (banHandler.isBanned(target, e.getServer().get())) {
            handler.reply("User ``" + target.getDiscriminatedName() + "`` (ID: " + target.getId() + ") is already currently banned from the server.");
            return;
        }

        boolean success = banHandler.banUser(target, e.getServer().get());

        if (success) {
            handler.reply("User ``" + target.getDiscriminatedName() + "`` (ID: " + target.getId() + ") has been banned from the server.");
        } else {
            handler.reply("An internal error has occurred while trying to ban user.");
        }
    }
}
